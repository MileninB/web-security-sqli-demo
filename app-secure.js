...
app.get('/', (req, res) => {
  res.send(`
    ...
    <head>
      <meta charset="UTF-8" />
      <title>SQL Injection Demo (защищённая версия)</title>
    </head>
    <body>
      <h1>Поиск пользователей — защищённая версия</h1>
      ...
      <ul>
        ...
        <li><a href="/search?username=%27%20UNION%20SELECT%20id%2Cusername%2Cpassword%2Cis_admin%20FROM%20users%20--%20">UNION: попытка атаки</a></li>
      </ul>
    </body>
    </html>
  `);
...
app.get('/search', (req, res) => {
  ...
  if (!/^[a-zA-Z0-9_]{1,30}$/.test(username)) {
    return res.status(400).send(`
      <h1>Ошибка</h1>
      <p>Недопустимые символы в имени пользователя.</p>
      <p><a href="/">Назад</a></p>
    `);
  }
...
app.listen(PORT, () => {
  console.log(`Защищённый сервер запущен на http://localhost:${PORT}`);
});